...
-- NOTE: we've used syntax now() - '3 days'::INTERVAL
-- this generates a valid datetie in SQL 
-- that's 3 days before right now
-- Use when seeding or performing queries on dates relative to current datetime

-- NOTE: we've "escaped" single quotes by using double single quotes
-- Ex: 'String's value' => doesn't work as string is single quotes & contains a single quote
-- Ex: 'String''s value' => works bcz we escape the single quote w. another single quote